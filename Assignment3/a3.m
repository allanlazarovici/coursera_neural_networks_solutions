...
function model = a3(wd_coefficient, n_hid, n_iters, learning_rate, momentum_multiplier, do_early_stopping, mini_batch_size)
...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  m = size(data.inputs, 2) %m is the number of datapoints (adopting the convention of cs229.stanford.edu)
	
  H_i = model.input_to_hid*data.inputs; %hidden input
  H_o = 1./( 1 + exp(-H_i) ); %hidden output- size is (num_hidden, m)
			     
  C_i = model.hid_to_class*H_o; %size is (num_classes, m). C_i s input to class layer

  C_o_denominators = log_sum_exp_over_rows(C_i); %size is (1, num_datapoints)
  C_o_log = C_i - repmat(C_o_denominators, [size(model.hid_to_class, 1), 1]); %size is (num_classes, num_datapoints)
  C_o = exp(C_o_log); %size is (num_classes, num_datapoints)

  dLoss_dC_i = (1/m)*(-data.targets + C_o);
  ret.hid_to_class = dLoss_dC_i*H_o' + wd_coefficient*model.hid_to_class;

  dLoss_dH_o = model.hid_to_class' * dLoss_dC_i;
  dLoss_dH_i = dLoss_dH_o .* (H_o .* (1 - H_o));
  ret.input_to_hid = dLoss_dH_i*data.inputs' + wd_coefficient*model.input_to_hid;
end
